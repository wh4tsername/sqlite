...
int sqlite3_wal_set_switch_callback(
  ...
  rc = sqlite3SetWalSwitch(db, iDb, callbackData, callback);
  sqlite3Error(db, rc);
...
// TODO docs
int sqlite3SetWalSwitch(sqlite3 *db, int iDb, void* callbackData, void (*callback)(void*, int, unsigned int)){
  int rc = SQLITE_OK;             /* Return code */
  int i;                          /* Used to iterate through attached dbs */
  int bBusy = 0;                  /* True if SQLITE_BUSY has been encountered */
  ...
  for(i=0; i<db->nDb && rc==SQLITE_OK; i++){
    if( i==iDb || iDb==SQLITE_MAX_DB ){
      ...
      rc = sqlite3PagerSetSwitchCallback(pager, callbackData, callback);
      if( rc==SQLITE_BUSY ){
        bBusy = 1;
        rc = SQLITE_OK;
      }
